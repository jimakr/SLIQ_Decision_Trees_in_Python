...
class Decisiontree:
    ...
    def choose_split(self, leafset, dataset, depth):
        best = 2
        best_split = 0
        best_list = []
        column_index = -1
    ...
    def train_tree(self, minleafexample, similarity_stop, maxdepth):
        ...
        root = self.choose_split(leaf, self.dataset, 1)  # makes first split
        ...
        while len(self.unfinished_nodes) > 0:
            ...
            for child in ['left', 'right']:  # for it's child lists
                if self.decide_class(node[child])[1] < similarity_stop and len(node[child]) > minleafexample and node[
                    'depth'] < maxdepth:  # checks condition
                    node[child] = self.choose_split(node[child], self.dataset, node['depth'])  # splits the child
    ...
    def print_the_tree(self, node, prefix=''):
        print(prefix[:-2] + "--Split with: " + str(node['split']) + ' at column ' + str(
            self.columnnames[node['column_split']]))
        prefix = prefix + '|  '
